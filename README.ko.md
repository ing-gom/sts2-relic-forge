...
강화된 유물은 일정 확률(기본 ~33%)로 **저주**를 추가로 얻습니다 — 지니는 동안 **엘리트·보스가 강해집니다**(유물 이름에 〈저주〉 표시, 툴팁에 정확한 효과). 강한 유물의 저주일수록 적도 더 강해지며, 여러 저주는 한 적에게 중첩됩니다. 전체 목록은 **[저주 가이드](CURSES.ko.md)** ([English](CURSES.en.md) · [简体中文](CURSES.zh.md)) 참고.

- 저주 예: 재앙(적 힘)·악의(판금)·원한(가시)·폭군(힘+판금+가시)·피(재생)·인공물·버퍼·광란(3턴마다 힘).
- **기본 켜짐** — ModConfig **"Enemy forge"** 로 조절합니다. 순수 파워 판타지로 즐기려면 **끄면** 됩니다.
- 강도·확률은 ModConfig 슬라이더로 조절합니다.
