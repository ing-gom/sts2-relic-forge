...
public class MainFile
{
    ...
    private static void RegisterConfig()
    {
        ...
        Loc(b, L("접두사 확률 (%)", "词缀几率 (%)"), L(
            "획득한 유물에 접두사가 붙을 확률 (순수 이득 — 저주 없음; 저주는 재련에서만 나옵니다). 기본값 15로 낮게 유지 — 유물은 본래 효과로 고르고, 접두사는 모닥불 재련으로 직접 만드는 설계입니다. 0 = 모든 획득 유물이 바닐라, 100 = 대상 유물 전부 접두사. 재련은 이 값과 무관하게 항상 접두사가 붙습니다.",
        ...
        Loc(b, L("적의 재련 (정예·보스의 반격)", "敌方重铸（精英与Boss的反击）"), L(
            "적 재련 메커니즘 마스터 스위치: 켜면 재련된 유물 일부가 정예·보스를 강화하는 '적 강화' 저주를 함께 지닙니다. 순수한 파워 판타지를 원하면 끄세요 (유물 재련 자체는 영향받지 않습니다).",
        ...
        Loc(b, L("저주 확률 (%)", "诅咒几率 (%)"), L(
            "재련된 유물이 저주 하나를 지닐 기준 확률 — 적 강화 저주(정예·보스 강화) 또는 자기 저주(막지 못한 피격 시 불이익) 중 하나이며, 둘이 겹치지 않습니다. 실제 확률은 접두사의 위력에 비례 — 약한 접두사는 저주가 드물고 전설급은 거의 항상 붙습니다. 종류 비율은 '자기 저주 비율'로 조절합니다. 재련하면 다시 굴립니다.",
        ...
        b.Dropdown(EntryKeyPrefixPool, "Prefix pool",
            ...
            .Description("Which prefixes can roll, on pickup and reforge alike. 'Enhance only' = prefixes that just strengthen the relic's own numbers (e.g. a Legendary Anchor blocks more) — for players who want vertical upgrades without new mechanics. 'Effects only' = prefixes that add mechanics (grafted relic effects, combat triggers, character affixes). 'Custom' = your own per-prefix / per-curse picks (edit them with the button below). 'All' (default) = everything. Applies to future forges; in co-op the HOST's setting applies to everyone.");
        Loc(b, L("접두사 풀", "词缀池"), L(
            "어떤 접두사가 롤될지 정합니다 (획득·재련 공통). 'Enhance only' = 유물 본체 수치만 강화하는 접두사 (예: 전설적인 닻은 블록만 더 줌) — 새 메커니즘 없이 수직 강화만 원하는 플레이어용. 'Effects only' = 메커니즘을 추가하는 접두사 (유물 효과 접합·전투 트리거·캐릭터 접두사). 'Custom' = 접두사·저주를 하나하나 직접 고른 나만의 풀 (아래 버튼으로 편집). 'All'(기본) = 전부. 이후의 재련부터 적용되며, co-op에서는 호스트 설정이 모두에게 적용됩니다.",
        ...
        try
        {
            ...
            if (btnM != null)
            {
                ...
                b.Description("Opens the editor for the Custom pool: every prefix and curse as its own on/off switch. The picks apply while 'Prefix pool' is set to Custom.");
                Loc(b, L("접두사·저주 커스텀", "自定义词缀与诅咒"), L(
                    "Custom 풀 편집기를 엽니다: 모든 접두사·저주를 각각 켜고 끌 수 있습니다. '접두사 풀'이 Custom일 때 적용됩니다.",
        ...
        Loc(b, L("고대 유물 재련", "重铸远古遗物"), L(
            "고대(Ancient) 유물도 다른 유물처럼 접두사를 받을지 여부. 끄면 고대 유물은 순수 바닐라로 남습니다 — 획득 재련을 건너뛰고 재련 선택 목록에서도 숨겨집니다. 이후 재련(또는 로드 시 재파생)되는 유물부터 적용됩니다.",
        ...
        Loc(b, L("자기 저주 비율 (%)", "自我诅咒占比 (%)"), L(
            "저주가 붙는 유물 중('저주 확률' 참고) 적 강화 저주 대신 자기 저주 — 막지 못한 피격 시 자신에게 약화/손상/취약 또는 상태이상 카드 — 를 받는 비율. 0 = 모든 저주가 적을 강화, 100 = 모든 저주가 나를 벌합니다. 한 유물에 둘이 겹치지 않습니다.",
